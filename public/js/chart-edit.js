var ChartEdit = function () {
    ...
    $("#create_chart").on('click',function(){
        ...
        switch(selected_text)
        {
            case "pie chart":
                {                        
                    config = pie_chart_draw(content, "");

                    break;
                }  
            case "horizontal chart":
                {
                    config = horizontal_chart_draw(content, "");
                    break;
                }                
                
            case "stacked chart":
                {
                    config = stacked_chart_draw(content, "");
                    break;
                } 
            case "vertical chart":
                {
                    config = vertical_chart_draw(content, "");
                    break;
                }

            case "line chart":
            {
                config = line_chart_draw(content, "")
                break;
            }

            case "point-styling chart":
            {
                config = pointstyle_chart_draw(content, "")
                break;
            }

            case "bubble chart":
            {
                config = bubble_chart_draw(content, "")
                break;
            }

            case "combo-bar-line chart":
            {
                config = combo_line_chart_draw(content, "");
                break;
            }

            case "doughnut chart":
            {
                config = doughnut_chart_draw(content, "");
                break;
            }

            case "multi-series-pie chart":
            {
                config = multipie_chart_draw(content, "");
                break;
            }

            case "polar-area chart":
            {
                config = polar_area_chart_draw(content, "");
                break;
            }
            case "radar-chart":
            {
                config = radar_chart_draw(content, "");
                break;
            }

            case "scatter chart":
            {
                config = scatter_chart_draw(content, "");
                break;
            }

            case "radar1-chart":
            {
                config = radar1_chart_draw(content, "");
                break;
            }
            case "table":
                {   
...
function initAttr()
{
    $('.attr-li').removeClass('active');
    $('#chart-div').hide();
    $('#series-div').hide();
}
let serisearray = ['database1', 'database2'];
function drawSeriesDiv()
{
    let strHtml = '';
    for(i=0; i < serisearray.length; i++)
    {
        strHtml += '<li>' + serisearray[i] + '</li>';
    }
    $('#series-list').html(strHtml);
}
...
function drawSettingList()
{
    ...
    {
        strHtml += "<li>\n" +
                        "<label class=\"col-12\">Serie</label>\n" +
                        "<input class=\"col-12 form-control \" id=\"label_" + (i+1) + "\" type=\"text\" value=\"database1\">\n" +
                        "<label class=\"col-12\">Color</label>\n" +
                        "<input class=\"col-12 form-control input-small\" type=\"color\" id=\"color_" + (i+1) + "\" value=\"#ff4323\">\n" +
                    "</li>";
    }

    $('#setting-list').html(strHtml);
}

$('#chart-attr').on('click', function () {
    initAttr();
    $('#chart-attr').addClass('active');
    drawSettingList();
    $('#chart-div').show();
});

$('#series-attr').on('click', function () {
    initAttr();
    $('#series-attr').addClass('active');
    drawSeriesDiv();
    $('#series-div').show();
});
